...
/* XXX: This function should be merged with driver's EphyrPreInit(). */
void
...
#define BUF_LEN 256
        ...
        snprintf(buf, BUF_LEN, "Xorg on %s.%d%s%s",
                 HostX.server_dpy_name ? HostX.server_dpy_name : ":0",
                 scrpriv->mynum, (extra_text != NULL) ? " " : "",
...
hostx_set_cmap_entry(ScreenPtr pScreen, unsigned char idx,
    ...
    ScrnInfoPtr screen = pScreenPriv->screen;
    EphyrScrPriv *scrpriv = screen->driverPrivate;
...
void
hostx_close_screen(ScrnInfoPtr screen) {
    EphyrScrPriv *scrpriv = screen->driverPrivate;
...
Bool
hostx_load_keymap(KeySymsPtr keySyms,
                  CARD8 *modmap,
    ...
    if (!use_r) {
        EPHYR_DBG("Couldn't use XKB extension.");
        return FALSE;
    } else if (!use_r->supported) {
        EPHYR_DBG("XKB extension is not supported in X server.");
        free(use_r);
        return FALSE;
    } else {
        free(use_r);
        ...
        if (!controls_r) {
            EPHYR_DBG("Couldn't get XKB keyboard controls.");
            return FALSE;
        }
